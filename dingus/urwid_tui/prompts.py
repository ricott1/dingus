    ...
    def __init__(
        ...
        if cancel_callback:
            footer_columns.append(
                urwid.LineBox(
                    attr_button("Cancel", on_press=self.cancel, borders=False)
                )
            )
...
class TextPrompt(Prompt):
    def __init__(
        self, error_text: str, title: str, ok_callback: Callable[[], None]
    ) -> None:
...
class NewAccountPrompt(Prompt):
    def __init__(
        self,
        ok_callback: Callable[[str, str], None],
        ...
        name_padding = urwid.Padding(name_filler, left=1, right=1)
        ...
        pwd_padding = urwid.Padding(pwd_filler, left=1, right=1)
...
class ImportPrompt(Prompt):
    def __init__(
        self,
        ok_callback: Callable[[str, str, str], None],
        cancel_callback: Callable[[], None],
    ) -> None:
        self.name_edit = urwid.Edit("", edit_text="MyAccount #")
        name_filler = urwid.Filler(self.name_edit, valign="top")
        ...
        passphrase_padding = urwid.Padding(passphrase_filler, left=1, right=1)
...
class SendPrompt(Prompt):
    def __init__(
        self, ok_callback: Callable[[dict], None], cancel_callback: Callable[[], None]
    ) -> None:
