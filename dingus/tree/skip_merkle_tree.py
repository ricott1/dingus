...
class SkipMerkleTree(object):
    ...
    async def _update(
        ...
        await self.set_subtree(new_leaf)
        ...
        if isinstance(current_node, EmptyNode):
            ...
            await self.set_subtree(bottom_node)
    ...
    async def update_subtree(
        ...
        bins = [
            [(key, value) for key, value in zip(keys, values) if key[height] == bin]
            for bin in range(256)
        ]
        await self._db.delete(current_subtree.hash)

        # for h, current_node in zip(current_subtree.structure, current_subtree.nodes):
        _nodes = []
        _structure = []
        for i in range(len(current_subtree.nodes)):
            ...
            bin_idx = split_index(
                keys, lambda k: all([is_bit_set(k, height + j) for j in range(h)])
            )
            bin_keys, keys = keys[:bin_idx], keys[bin_idx:]
            bin_values, values = values[:bin_idx], values[bin_idx:]
            # If the node is a branch, it necessarily is at the bottom layer of the subtree
            if isinstance(current_node, BranchNode):
                assert h == self.subtree_height - 1
                ...
                left_subtree = await self.update_subtree(
                    bin_keys[i],
                    bin_value[i],
                    left_subtree,
                    height + self.subtree_height,
                )
                right_subtree = await self.update_subtree(
                    bin_keys[i],
                    bin_value[i],
                    left_subtree,
                    height + self.subtree_height,
                )
                node = BranchNode(left_subtree.hash, right_subtree.hash)
                _structure.append(h)
                _nodes.append(node)
            else:
                while h < self.subtree_height:
                    ...
                    node = BranchNode(left_node.hash, right_node.hash)
                    h += 1
                _nodes.append(node)
                _structure.append(h)

        await self.set_subtree(current_subtree)
        return current_subtree

        # keep structure as height without repeating it. Then loop through structure, and depending on the height bundle together many bins (e.g., if height=0 bundle them all)
        # recall this same function. The only real return values (non recursive) are given when you reach the bottom of the subtree (height//SUBTREE_HEIGHT) in which case
        # you return the leaf, or the bottom of the tree, in which case you still return the leaf. This happens when len(keys) == 1 or 0
    ...
    async def _update_batch(
        ...
        for h, node, bin in zip(current_subtree.structure, current_subtree.nodes, bins):
            if isinstance(current_subtree, BranchNode):
                ...
                await self._db.delete(current_subtree.hash)
                left_subtree = await self._update_batch(
                    bin[0], bin[1], left_subtree, height + 1
                )
                right_subtree = await self._update_batch(
                    bin[0], bin[1], right_subtree, height + 1
                )
        ...
        await self.set_subtree(current_node)
        return current_node
